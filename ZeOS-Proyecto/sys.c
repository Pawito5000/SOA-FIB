...
int sys_threadCreate(void (*function_wrap)(void*, void*), void (*function)(void*), void* parameter)
{
	...
	if(free != -1){
		...
		int logical_page = free_pages[free];  // Pàgina logica
		char *logical_address = (char *)(logical_page << 12); 
		...
		if(current()->heap_end_ptr > logical_address) {
			...
			struct list_head *pos;
                        list_for_each(pos, current()->threads_process){
                                struct task_struct *ts = list_head_to_task_struct(pos);
                                ts->heap_end_ptr = logical_address;
                        }
		}
...
void sys_threadExit(void)
{
	if((current()->PID == 1) && (current()->TID == 1)) sys_exit();
		...
		struct list_head *n;
		list_for_each_safe(head, n, current()->threads_process){
